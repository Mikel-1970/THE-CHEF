import {loadBrandImage,drawBrand} from './brand';
...
export async function createSocialCard(recipe:Recipe,servings:number,format:SocialFormat,imageUrl:string):Promise<File>{
 ...
 const h=canvas.height;ctx.fillStyle='#fff7e9';ctx.fillRect(0,0,1080,h);
 const text=(value:string,x:number,y:number,size:number,color='#713817',serif=false)=>{ctx.fillStyle=color;ctx.font=`${serif?'700':'600'} ${size}px ${serif?'"Playfair Display", Georgia':'"DM Sans", sans-serif'}`;ctx.fillText(value,x,y);};
 const top=format==='story'?160:64;
 drawBrand(ctx,await loadBrandImage(),64,top-40,400);
