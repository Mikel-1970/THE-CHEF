...
import { techniqueMaster } from '../data/techniqueMaster';
...
export async function initializeCulinaryCatalog():Promise<void>{
 ...
 try{
  ...
  if(revision!=='2026-09-23.1')await new Promise<void>((resolve,reject)=>{const tx=db.transaction(['techniques','tips','metadata'],'readwrite');tx.objectStore('techniques').clear();tx.objectStore('tips').clear();techniqueMaster.forEach(t=>tx.objectStore('techniques').put(t));cookingTips.forEach(t=>tx.objectStore('tips').put(t));tx.objectStore('metadata').put('2026-09-23.1','revision');tx.oncomplete=()=>resolve();tx.onerror=()=>reject(tx.error);tx.onabort=()=>reject(tx.error)});
  ...
  if(techniques.length===techniqueMaster.length&&tips.length===cookingTips.length){techniqueMaster.splice(0,techniqueMaster.length,...techniques);cookingTips.splice(0,cookingTips.length,...tips);}
 }finally{db.close()}
}
