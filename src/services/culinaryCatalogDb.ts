...
export async function initializeCulinaryCatalog():Promise<void>{
 ...
 try{
  ...
  if(revision!=='2026-09-22.1')await new Promise<void>((resolve,reject)=>{const tx=db.transaction(['techniques','tips','metadata'],'readwrite');tx.objectStore('techniques').clear();tx.objectStore('tips').clear();techniqueBasics.forEach(t=>tx.objectStore('techniques').put(t));cookingTips.forEach(t=>tx.objectStore('tips').put(t));tx.objectStore('metadata').put('2026-09-22.1','revision');tx.oncomplete=()=>resolve();tx.onerror=()=>reject(tx.error);tx.onabort=()=>reject(tx.error)});
