...
export function getTechniquesForRecipe(recipe:Recipe):Technique[]{
  ...
  return unique([...explicit,...recipe.steps.flatMap(getTechniquesForStep)]).slice(0,10);
}
...
export function inferTechniquesFromText(text:string,limit=3):Technique[]{
  ...
  if(!value)return [];
  const verbs:Record<string,string>={sofrie:'sofreir',sofria:'sofreir',frie:'freir',fria:'freir',cuece:'cocer',cueza:'cocer',hierve:'hervir',hierva:'hervir',tuesta:'tostar',tueste:'tostar',muele:'moler',muela:'moler',bate:'batir'};
  const tokens=value.split(/[^a-z0-9]+/).filter(Boolean).map(token=>verbs[token]||token);
  // Ingredient keywords alone (water, oil, meat...) must not invent a technique.
  const matches=searchIndex.map(entry=>({technique:entry.technique,score:
    [entry.technique.title,...(entry.technique.aliases||[])].map(normalize)
      .filter(term=>matchesTerm(value,tokens,term)).reduce((score,term)=>Math.max(score,term.length),0)
  })).filter(entry=>entry.score>0).sort((a,b)=>b.score-a.score);
  return unique(matches.map(entry=>entry.technique)).slice(0,limit);
}

function matchesTerm(text:string,tokens:string[],term:string){
  if(term.length>=4&&(` ${tokens.join(' ')} `).includes(` ${term} `))return true;
