...
export const HYBRID_NOTICE_STORAGE_KEY='the-chef:last-hybrid-notice'; const PROPOSAL_COUNT=1; const DIFFICULTY_RANK:Record<Proposal['difficulty'],number>={'Fácil':1,'Media':2,'Avanzada':3};
...
export async function getHybridProposals(request:CookingRequest,excludeRecipeIds:string[]=[]):Promise<HybridRecommendationResult>{
 const preference=Math.max(0,Math.min(100,request.aiPreference??100)); const aiCount=Math.max(0,Math.min(PROPOSAL_COUNT,Math.round(preference/100))); const localCount=PROPOSAL_COUNT-aiCount; let externalError:string|undefined; let aiPool:Proposal[]=[];
 if(aiCount>0&&isAiProposalApiConfigured()){try{aiPool=(await fetchAiProposals(request)).filter(p=>proposalWithinLimits(p,request)).slice(0,PROPOSAL_COUNT)}catch(e){externalError=e instanceof Error&&e.message?e.message:'No se ha podido consultar la IA.'}}
 ...
 const finalProposals=proposals.slice(0,PROPOSAL_COUNT); if(!finalProposals.length)throw new Error('No hay recetas que cumplan los límites indicados. Amplía tiempo, dificultad o revisa las restricciones.'); if(finalProposals.length<PROPOSAL_COUNT&&!externalError)externalError='Solo se ha encontrado una propuesta que cumple todos tus límites.'; const usesAi=finalProposals.some(p=>aiPool.some(a=>a.id===p.id)); persistEngineNotice(externalError);return{proposals:finalProposals,mode:usesAi?'hybrid':'local',externalRecipesAdded:0,externalError};
}
...
function proposalWithinLimits(proposal:Proposal,request:CookingRequest){if(request.maxMinutes&&proposal.minutes>request.maxMinutes)return false;if(request.difficulty&&DIFFICULTY_RANK[proposal.difficulty]>DIFFICULTY_RANK[request.difficulty])return false;return true}
