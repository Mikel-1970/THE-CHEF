...
export const HYBRID_NOTICE_STORAGE_KEY='the-chef:last-hybrid-notice'; const PROPOSAL_COUNT=2; const DIFFICULTY_RANK:Record<Proposal['difficulty'],number>={'Fácil':1,'Media':2,'Avanzada':3};
...
export async function getHybridProposals(request:CookingRequest,excludeRecipeIds:string[]=[]):Promise<HybridRecommendationResult>{
 ...
 if(preference>0&&isAiProposalApiConfigured()){try{aiPool=(await fetchAiProposals(request)).filter(p=>proposalWithinLimits(p,request)).slice(0,PROPOSAL_COUNT)}catch(e){externalError=e instanceof Error&&e.message?e.message:'No se ha podido consultar la IA.'}}
