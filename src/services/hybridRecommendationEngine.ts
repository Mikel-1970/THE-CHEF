...
export async function getHybridProposals(request:CookingRequest,excludeRecipeIds:string[]=[]):Promise<HybridRecommendationResult>{
 const preference=Math.max(0,Math.min(100,request.aiPreference??100)); const aiCount=Math.max(0,Math.min(PROPOSAL_COUNT,Math.round(preference/50))); const localCount=PROPOSAL_COUNT-aiCount; let externalError:string|undefined; let ai:Proposal[]=[];
