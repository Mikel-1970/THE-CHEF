...
export function CookingOptions({value,onChange,expanded=false,hideServings=false,hideMaxMinutes=false}:{value:CookingOptionsValue;onChange:(patch:Partial<CookingOptionsValue>)=>void;expanded?:boolean;hideServings?:boolean;hideMaxMinutes?:boolean}) {
 ...
 return <>
   ...
   {(expanded||optionsOpen)&&<section className="visual-options" aria-label="Personalizar receta">
    {!hideServings&&<div className="visual-option"><div className="visual-option-title"><UsersRound/><strong>Comensales</strong></div><NumberStepper value={servings} onChange={v=>{setServings(v)}}/></div>}
    {!hideMaxMinutes&&<div className="visual-option"><div className="visual-option-title"><Clock3/><strong>Tiempo máximo</strong></div>{maxMinutes===undefined?<button type="button" className="visual-option-enable" onClick={()=>setMaxMinutes(60)}><span>Sin límite</span><small>Activar límite</small></button>:<div className="visual-option-active-control"><NumberStepper value={maxMinutes} min={15} max={120} step={5} suffix="min" editable onChange={v=>setMaxMinutes(v)}/><button type="button" className="visual-option-clear" onClick={()=>setMaxMinutes(undefined)}>Quitar límite</button></div>}</div>}
    <label className="visual-option"><span className="visual-option-title"><Palette/><strong>Estilo</strong></span><select aria-label="Estilo" value={style??''} onChange={e=>setStyle(e.target.value||undefined)}><option value="">Indiferente</option>{RECIPE_STYLES.filter(v=>v!=='Indiferente').map(v=><option key={v} value={v}>{v}</option>)}</select></label>
    ...
    <label className="visual-option"><span className="visual-option-title"><Gauge/><strong>Dificultad máxima</strong></span><select aria-label="Dificultad máxima" value={difficulty??''} onChange={e=>setDifficulty(e.target.value as Difficulty||undefined)}><option value="">Indiferente</option>{['Fácil','Media','Avanzada'].map(v=><option key={v} value={v}>{v}</option>)}</select></label>
    <label className="visual-option"><span className="visual-option-title"><ChiliIcon/><strong>Picante</strong></span><select aria-label="Picante" value={spiceLevel??''} onChange={e=>setSpiceLevel((e.target.value||undefined) as SpiceLevel|undefined)}><option value="">Sin aplicar</option>{['Nada','Suave','Medio','Alto'].map(v=><option key={v} value={v}>{v}</option>)}</select></label>
   </section>}
 </>;
}
