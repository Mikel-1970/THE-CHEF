...
export function SettingsAbout({url}:{url:string}){
 ...
 const share=async(text:string,link?:string)=>{try{if(navigator.share)await navigator.share({title:'The Chef',text,...(link?{url:link}:{})});else{await navigator.clipboard.writeText([text,link].filter(Boolean).join('\n'));setStatus('Copiado. Ya puedes compartirlo.')}}catch(e){if(!(e instanceof Error&&e.name==='AbortError'))setStatus('No se pudo compartir. Puedes copiar el texto manualmente.')}};
 ...
 <button onClick={()=>void share('Prueba The Chef. El acceso a esta preview sigue siendo privado.',url)}><Share2/>Compartir la app</button>
