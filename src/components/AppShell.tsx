import { CalendarDays, ArrowLeft, BookOpen, Camera, CookingPot, Heart, Home, PackageOpen, Search, Settings, ShoppingBasket, Sparkles, X } from 'lucide-react';
...
export function AppShell({children,hideProfile=false,hideBack=false,onBack}: {children:ReactNode;hideNav?:boolean;hideBack?:boolean;hideProfile?:boolean;onBack?:()=>void}) {
 ...
 return <div className="app-bg chef-app-shell">
  ...
  {createPortal(<div className="chef-navigation">
   ...
   {open&&<><button className="chef-menu-backdrop" aria-label="Cerrar navegación" onClick={()=>setOpen(false)}/><nav id="chef-navigation-panel" className="chef-navigation-panel" aria-label="Menú de navegación"><div className="chef-menu-heading"><strong>Tu cocina</strong><button aria-label="Cerrar menú de navegación" onClick={()=>{setOpen(false);button.current?.focus()}}><X size={20}/></button></div><div className="chef-menu-grid">{items.map(([path,label,Icon])=><button key={path} aria-label={label} disabled={path==='/plan-semanal'} title={path==='/plan-semanal'?'Próximamente':undefined} onClick={()=>go(path)}><Icon size={23}/><span>{label}</span>{path==='/plan-semanal'&&<small>Próximamente</small>}</button>)}</div></nav></>}
  </div>,document.body)}
 </div>
}
