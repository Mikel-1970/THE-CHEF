...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const setSearch = (request: CookingRequest, nextProposals: Proposal[]) => {
    ...
    setHistory(current => { const next = [entry, ...current].slice(0, 30); saveHistory(next); return next; });
  };
  const replaceProposals = (nextProposals: Proposal[]) => { setProposals(nextProposals); saveActiveSearch(currentRequest, nextProposals); };
  const toggleFavorite = (recipeId: string) => setFavorites(current => { const next = current.includes(recipeId) ? current.filter(id => id !== recipeId) : [recipeId, ...current]; saveFavorites(next); return next; });
  const toggleSavedRecipe = (recipeId: string) => setSavedRecipes(current => { const next = current.includes(recipeId) ? current.filter(id => id !== recipeId) : [recipeId, ...current]; saveSavedRecipes(next); return next; });
  ...
  const recordRecipeView = (recipeId: string, recipeTitle: string) => {
    ...
    setHistory(current => { const next = [entry, ...current.filter(item => !(item.kind === 'recipe' && item.recipeId === recipeId))].slice(0, 30); saveHistory(next); return next; });
    // La compra general permanece; solo se descartan pendientes automáticos de otra receta.
    setShoppingList(current => {
      const next = current.filter(item => !item.recipeId || item.recipeId === recipeId);
      if (next.length !== current.length) saveShoppingList(next);
      return next.length === current.length ? current : next;
    });
  };
  const removeHistoryEntry = (entryId: string) => setHistory(current => { const next = current.filter(item => item.id !== entryId); saveHistory(next); return next; });
  const removeRecipeFromLibrary = (recipeId: string) => { setSavedRecipes(current => { const next = current.filter(id => id !== recipeId); saveSavedRecipes(next); return next; }); setFavorites(current => { const next = current.filter(id => id !== recipeId); saveFavorites(next); return next; }); };
  ...
  const upsertShoppingItem = (item: ShoppingListItem) => {
    setShoppingList(current => {
      ...
      if (sameNameIndex >= 0) {
        ...
        const next = [...current]; next[sameNameIndex] = nextItem; saveShoppingList(next); return next;
      }
      const next = [item, ...current]; saveShoppingList(next); return next;
    });
  };
