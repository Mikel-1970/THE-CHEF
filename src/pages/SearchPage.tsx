...
export function SearchPage() {
  ...
  const recipes = useMemo(() => {
    ...
    if (!words.length) return mockRecipes;
          ...
          <div className="library-list">
            {recipes.map(recipe => (
              <button className="library-card" key={recipe.id} onClick={() => navigate(`/receta/${recipe.id}`)}>
                ...
                <div><strong>{recipe.title}</strong><small><Clock3 size={13} /> {formatDuration(recipe.prepMinutes + recipe.cookMinutes)} · {recipe.cuisine} · {recipe.style}</small></div>
              </button>
            ))}
