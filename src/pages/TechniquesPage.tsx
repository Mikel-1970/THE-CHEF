  ...
  return (
    <AppShell>
      ...
      <div className="page-content nav-safe techniques-page">
        ...
        <section className="library-section techniques-library"><div className="section-heading-row"><div><span className="eyebrow">BIBLIOTECA MAESTRA</span><h2>{saved.length ? `${saved.length} técnicas disponibles` : 'Todavía vacío'}</h2></div></div><input aria-label="Buscar técnica o uso" placeholder="Buscar técnica, alias o uso: saltear, wok, salsas…" value={filter} onChange={e=>setFilter(e.target.value)}/><div className="technique-filter-row"><select aria-label="Categoría de técnica" value={category} onChange={e=>setCategory(e.target.value)}><option>Todas</option>{Array.from(new Set(saved.map(t=>t.category))).map(c=><option key={c}>{c}</option>)}</select><select aria-label="Nivel de técnica" value={level} onChange={e=>setLevel(e.target.value)}><option>Todos</option><option>Fácil</option><option>Media</option><option>Avanzada</option></select></div><div className="library-list">{saved.filter(t=>(category==='Todas'||t.category===category)&&(level==='Todos'||t.difficulty===level)&&[t.title,t.category,...t.uses,...(t.aliases||[]),...(t.keywords||[])].join(' ').toLocaleLowerCase('es').includes(filter.toLocaleLowerCase('es'))).map(technique => <div className="library-card technique-library-card" key={technique.id}><button type="button" className="technique-open" onClick={() => selectTechnique(technique)}><span className="library-emoji">🧪</span><div><strong>{technique.title}</strong><small>{technique.difficulty} · {technique.category}</small></div></button>{!technique.builtin&&<button type="button" className="icon-button" onClick={() => { removeTechnique(technique.id); setSaved(getTechniqueLibrary()); if (current?.id === technique.id) setCurrent(undefined); }} aria-label={`Eliminar ${technique.title}`}><Trash2 size={17} /></button>}</div>)}{!saved.length && <div className="empty-card">Pide al Chef una técnica y se guardará aquí automáticamente.</div>}</div></section>
      </div>
