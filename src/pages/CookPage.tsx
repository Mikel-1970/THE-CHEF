...
export function CookPage() {
  ...
  useEffect(() => {
    if (!recipe) return;
    if (sourcePhotoUrl) {
      setReferenceImageUrl(sourcePhotoUrl);
    ...
    getRecipeImage(recipe)
      .then(url => { generatedUrl = url; if (!disposed && url) setReferenceImageUrl(url); })
      .catch(() => { if (!disposed) setReferenceImageError(true); });
    return () => {
      disposed = true;
      if (generatedUrl?.startsWith('blob:')) URL.revokeObjectURL(generatedUrl);
    };
  }, [recipeId, sourcePhotoUrl]);

  if (!recipe) {
    return <CookFrame><header className="cook-header"><span /><div><span>Modo Cocina</span><strong>Receta no disponible</strong></div><span /></header><main className="cook-main"><p className="cook-instruction">No he podido recuperar esta receta. Vuelve a abrirla desde Mis recetas o genera una nueva propuesta.</p></main></CookFrame>;
  }
      ...
      <CookFrame finish>
        ...
        <main className="cook-finish-main">
          ...
          <section className="dish-reference-card" aria-label={sourcePhotoUrl ? 'Fotografía original' : 'Presentación esperada'}>
            ...
            {referenceImageUrl ? <img src={referenceImageUrl} alt={sourcePhotoUrl ? 'Plato original a reproducir' : `Presentación sugerida de ${recipe.title}`} /> : <div className="dish-reference-placeholder"><span>{recipe.emoji}</span><strong>{referenceImageError ? 'No se ha podido cargar la referencia' : 'Preparando la imagen en segundo plano…'}</strong></div>}
          </section>
  ...
  return (
    <CookFrame>
      ...
      <main className="cook-main">
        ...
        <div className="cook-step-facts">{step.temperatureC && <div><Thermometer size={18} /><span><small>Temperatura</small><strong>{step.temperatureC} °C</strong></span></div>}{step.minutes && <div><Clock3 size={18} /><span><small>Duración de este paso</small><strong>{step.minutes} min</strong></span></div>}</div>
        {step.cue && <div className="cook-cue"><ScreenShare size={18} /><span><strong>Fíjate en esto</strong>{step.cue}</span></div>}
        {step.minutes && <div className="timer-card timer-card-labelled"><Clock3 size={19} /><div><small>Temporizador de este paso</small><div className="timer-time">{mins}:{secs}</div></div><div className="timer-actions"><button type="button" onClick={() => setRunning(value => !value)} aria-label={running ? 'Pausar temporizador' : 'Iniciar temporizador'}>{running ? <Pause size={19} fill="currentColor" /> : <Play size={19} fill="currentColor" />}</button><button type="button" onClick={() => { setSeconds((step.minutes || 0) * 60); setRunning(false); }} aria-label="Reiniciar temporizador"><RotateCcw size={18} /></button></div></div>}
        <div className="cook-serving">Receta ajustada para <strong>{servings} personas</strong></div>
      </main>
      <footer className="cook-footer"><button className="cook-nav secondary" disabled={safeIndex === 0} onClick={() => setIndex(i => Math.max(0, i - 1))}><ChevronLeft size={20} /> Anterior</button>{safeIndex < recipe.steps.length - 1 ? <button className="cook-nav primary" onClick={() => setIndex(i => Math.min(recipe.steps.length - 1, i + 1))}>Siguiente <ChevronRight size={20} /></button> : <button className="cook-nav primary" onClick={() => setFinished(true)}><Sparkles size={18} /> Terminar</button>}</footer>
    </CookFrame>
  );
}

function CookFrame({ children, finish = false }: { children: ReactNode; finish?: boolean }) {
  return <><div className={`cook-screen with-bottom-nav ${finish ? 'cook-finish-screen' : ''}`}>{children}</div><BottomNav /></>;
}
