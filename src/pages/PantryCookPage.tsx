...
export function PantryCookPage() {
 ...
 const [selected,setSelected]=useState<Set<string>>(()=>new Set((previous?.pantryIngredients??[]).filter(i=>i.priority).slice(0,4).map(i=>normalize(i.name))));
 ...
 const [photoLocation,setPhotoLocation]=useState<StockLocation>('fridge');
 ...
 const toggle=(name:string)=>{const key=normalize(name);setSelected(current=>{const next=new Set(current);if(next.has(key))next.delete(key);else if(next.size<4)next.add(key);return next})};
 const addItems=(items:IngredientInput[])=>{
  ...
  setSelected(current=>new Set([...current,...items.map(i=>normalize(i.name))].slice(0,4)));
 };
 const addManual=(e:FormEvent)=>{e.preventDefault();if(voice.isListening||voice.isTranscribing)return;const items=draft.split(/[,;\n]+|\s+y\s+/i).map(s=>parseIngredientInput(s.trim())).filter(i=>i.name);addItems(items.map(i=>({...i,location:source==='fridge'?'fridge':'pantry'})));setDraft('')};
 ...
 const confirmPhoto=()=>{
  const items=candidates.filter(i=>i.checked&&i.name.trim()).map(i=>({name:i.name.trim(),location:photoLocation}));
  if(!items.length)return;
  addItems(items);setSelected(new Set(items.map(i=>normalize(i.name)).slice(0,4)));setCandidates([]);setPhoto(undefined);setSource('all');
 };
 const search=async()=>{
  if(!selectedItems.length||searching||analysing)return;setSearching(true);setError('');
  const available=pantry.map(i=>({...i,priority:selected.has(normalize(i.name))}));
 ...
 return <AppShell><ChefLoadingOverlay active={searching} title="Preparando tu propuesta" messages={['Cocinando con lo que tienes…']}/><TopBar title="Abre la despensa"/>
  <div className="page-content pantry-cook-visual">
   <p className="visual-hint">Elige 3 o 4 ingredientes.</p>
   ...
   <div className="pantry-ingredient-grid" aria-label="Ingredientes disponibles">{visible.map(item=>{const active=selected.has(normalize(item.name));return <button key={normalize(item.name)} aria-pressed={active} disabled={!active&&selected.size>=4} onClick={()=>toggle(item.name)}><span className="pantry-food-icon">{locationOf(item)==='fridge'?<Refrigerator/>:<PackageOpen/>}</span><strong>{item.name}</strong>{item.quantity!==undefined&&<small>{item.quantity} {item.unit}</small>}<span className="pantry-choice-mark">{active?<Check size={18}/>:<Star size={18}/>}</span></button>})}</div>
   ...
   <div className="pantry-selection-summary" role="status"><strong>{selectedItems.length}/4 elegidos</strong><span>{selectedItems.map(i=>i.name).join(' · ')}</span></div>
