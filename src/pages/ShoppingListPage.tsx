...
export function ShoppingListPage() {
  ...
  const addItems = (event?: FormEvent) => {
    ...
    entries.forEach(entry => {
      ...
      upsertShoppingItem({ id: `manual:${normalize(parsed.name).replace(/\s+/g, '-')}`, name: parsed.name, quantity: parsed.quantity, unit: parsed.unit, category: inferPantryCategory(parsed.name), checked: false });
    });
    if (entries.length) setDraft('');
  };

  const updateQuantity = (id: string, value: string) => {
    const item = shoppingList.find(entry => entry.id === id); if (!item) return;
  ...
  const addBoughtToStock = (item: ShoppingListItem, location: StockLocation) => {
    const stock = settings.pantryStock ?? [], index = stock.findIndex(entry => normalize(entry.name) === normalize(item.name));
    const nextIngredient: IngredientInput = { name: item.name, quantity: item.quantity, unit: item.unit, category: item.category ?? inferPantryCategory(item.name), location };
  ...
  return <AppShell>
    ...
    <div className="page-content nav-safe shopping-grouped-page">
      ...
      <div className="shopping-category-list">{grouped.map(([category, items]) => <section className="shopping-category" key={category}><div className="section-label"><span>{category}</span><small>{items.length}</small></div><div className="editable-stock-list">{items.map(item => <section className={`settings-card editable-stock-card shopping-stock-card ${item.checked ? 'checked' : ''}`} key={item.id}><button className="icon-button" onClick={() => toggleShoppingItem(item.id)} aria-label={item.checked ? 'Marcar pendiente' : 'Marcar comprado'}>{item.checked ? <Check size={20} /> : <Circle size={20} />}</button><div className="editable-stock-main"><strong className={item.checked ? 'strike' : ''}>{item.name}</strong>{item.recipeTitle && <small>Para {item.recipeTitle}</small>}<div className="quantity-unit-editor"><label><span>Cantidad</span><input inputMode="decimal" type="number" min={isCountUnit(item.unit)?1:0} step={isCountUnit(item.unit)?1:0.1} value={item.quantity ?? ''} placeholder="—" onChange={event => updateQuantity(item.id, event.target.value)} /></label><label><span>Unidad</span><select value={item.unit ?? ''} onChange={event => updateUnit(item.id, event.target.value)}><option value="">Sin indicar</option>{UNIT_OPTIONS.map(unit => <option value={unit} key={unit}>{unit}</option>)}</select></label></div>{item.checked && <div className="bought-stock-actions"><button type="button" onClick={() => addBoughtToStock(item, 'fridge')}><Snowflake size={15} /> Guardar en nevera</button><button type="button" onClick={() => addBoughtToStock(item, 'pantry')}><PackageOpen size={15} /> Guardar en despensa</button></div>}</div><button className="icon-button" onClick={() => removeShoppingItem(item.id)} aria-label="Eliminar de la lista"><Trash2 size={18} /></button></section>)}</div></section>)}</div>
...
function appendDictation(current: string, transcript: string) { const base = current.trimEnd(), clean = transcript.trim(); return base ? `${base}, ${clean}` : clean; }
