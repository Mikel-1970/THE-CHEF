...
function words(value:string){
 return new Set(normalize(value).split(/\s+/).filter(x=>x.length>=4));
}
function containsTrigger(context:string,value:string){
 const trigger=normalize(value);
 if(trigger.length<3)return false;
 if(context.includes(trigger))return true;
 const tokens=trigger.split(/\s+/).filter(x=>x.length>=4);
 return tokens.length>=2&&tokens.every(token=>context.includes(token));
}
function similarity(a:string,b:string){
 const aa=words(a),bb=words(b);
 if(!aa.size||!bb.size)return 0;
 let shared=0;for(const x of aa)if(bb.has(x))shared++;
 return shared/Math.min(aa.size,bb.size);
}
function duplicatesRecipeAdvice(tip:CookingTip,recipe:Recipe){
 const candidate=`${tip.title} ${tip.shortTip}`;
 return [...recipe.criticalPoints,...recipe.substitutions,...recipe.miseEnPlace].some(advice=>{
  const a=normalize(candidate),b=normalize(advice);
  return (a.length>18&&b.includes(a))||(b.length>18&&a.includes(b))||similarity(candidate,advice)>=0.72;
 });
}
function scoreTip(tip:CookingTip,recipe:Recipe,step:RecipeStep,stepIndex:number,reviews:TipReviews){
 const ingredientText=normalize(recipe.ingredients.map(i=>i.name).join(' '));
 const stepText=normalize([step.instruction,step.cue||'',recipe.title,recipe.style,recipe.cuisine].join(' '));
 const fullText=normalize([stepText,ingredientText].join(' '));
 let score=PRIORITY_SCORE[tip.priority];
 if(reviews[tip.id]==='keep')score+=2;
 score+=Math.min(18,tip.triggers.ingredients.filter(x=>containsTrigger(ingredientText,x)||containsTrigger(stepText,x)).length*9);
 score+=Math.min(14,tip.triggers.techniques.filter(x=>containsTrigger(stepText,x)).length*7);
 score+=Math.min(12,tip.triggers.actions.filter(x=>containsTrigger(stepText,x)).length*6);
 score+=Math.min(6,tip.triggers.equipment.filter(x=>containsTrigger(stepText,x)).length*3);
 score+=Math.min(6,tip.tags.filter(x=>containsTrigger(fullText,x)).length*2);
 score+=Math.min(6,tip.appliesTo.filter(x=>containsTrigger(ingredientText,x)).length*3);
 if(tip.useMoment.some(x=>normalize(x)==='durante la coccion'))score+=2;
