...
for(const [table,rows] of [['culinary_techniques',techniqueMaster],['culinary_tips',cookingTips]])for(const row of rows)sql+=`\nINSERT INTO ${table} (id,payload) VALUES (${quote(row.id)},${quote(JSON.stringify(row))}) ON CONFLICT(id) DO UPDATE SET payload=excluded.payload;`;
fs.mkdirSync('work',{recursive:true});fs.writeFileSync('work/catalog-seed.sql',sql);
console.log(`${techniqueMaster.length} técnicas; ${cookingTips.length} tips. SQL: work/catalog-seed.sql`);
