...
test('empty request asks for cuisine without calling AI',async({page})=>{let external=0;await page.route('**/*',r=>{if(new URL(r.request().url()).hostname==='127.0.0.1')return r.continue();if(new URL(r.request().url()).pathname.includes('/recipes/'))external++;return r.abort()});await page.goto('./#/antojo');await page.getByRole('button',{name:'Generar receta',exact:true}).click();await expect(page.getByRole('alert')).toHaveText(CUISINE_REQUIRED);await expect(page.getByLabel('Tipo de cocina',{exact:true})).toBeVisible();expect(external).toBe(0)});
test('chef choice sends cuisine, diners and directive to AI',async({page})=>{
 let body:any;await page.route('**/*',async r=>{const url=new URL(r.request().url());if(url.hostname==='127.0.0.1')return r.continue();if(url.pathname.endsWith('/recipes/suggest')){body=r.request().postDataJSON();return r.fulfill({status:200,contentType:'application/json',body:JSON.stringify({proposals:[]})})}return r.abort()});
 ...
 await page.getByRole('button',{name:'Generar receta',exact:true}).click();await expect.poll(()=>body).toBeTruthy();expect(body.request.cuisine).toBe('Peruana');expect(body.request.desireText).toContain('plato típico y representativo');expect(body.request.servings).toBe(5);
});
