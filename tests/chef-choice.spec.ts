...
test('chef choice opens a Peruvian library recipe for five without AI',async({page})=>{
 ...
 await page.getByRole('button',{name:'Generar receta',exact:true}).click();await expect(page).toHaveURL(/receta\/lib-\d+\?servings=5/);await expect(page.locator('.hero-meta')).toContainText('5 comensales');await expect(page.locator('.hero-meta')).toContainText('Peruana');expect(external.filter(path=>path.includes('/recipes/')||path.includes('/chef-media/'))).toEqual([]);
});
