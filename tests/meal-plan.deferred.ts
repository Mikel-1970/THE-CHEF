...
test('menu opens planner, no-goal plan survives navigation and shopping updates without duplicates',async({page},info)=>{
 ...
 await page.getByLabel('Cómo generar el plan',{exact:true}).selectOption('library');await page.getByLabel('Duración',{exact:true}).selectOption('4');await page.getByLabel('Comensales',{exact:true}).selectOption('5');await page.getByRole('button',{name:'Crear plan de comidas',exact:true}).click();
...
test('calculation never adopts a goal; partial-day plan and weight remain independent',async({page})=>{
 await page.goto('./#/plan-semanal');await page.getByLabel('Cómo generar el plan',{exact:true}).selectOption('library');await page.getByLabel('Objetivo energético',{exact:true}).selectOption('goal');await page.getByText('Calcular mantenimiento estimado (opcional)',{exact:true}).click();
...
test('AI plan generates breakfasts with chosen macros and reviews replacements before applying',async({page})=>{
 const payloads:any[]=[];await page.route('**/recipes/generate',async r=>{const payload=r.request().postDataJSON();payloads.push(payload);const recipe={...planBreakfasts[0],id:'ai-test',title:'Desayuno propuesto '+payloads.length,description:'Alternativa similar con avena.',source:{kind:'ai',label:'test'}};await r.fulfill({status:200,json:{recipes:[recipe]}})});
 await page.goto('./#/plan-semanal');await page.getByLabel('Comida',{exact:true}).uncheck();await page.getByLabel('Cena',{exact:true}).uncheck();await page.getByLabel('Desayuno',{exact:true}).check();await page.getByLabel('Objetivo energético',{exact:true}).selectOption('goal');await page.getByLabel('Mi objetivo (kcal/día por persona)',{exact:true}).fill('2000');await page.getByLabel('Distribución de nutrientes',{exact:true}).selectOption('percent');await page.getByLabel('Proteínas (%)',{exact:true}).fill('20');await page.getByLabel('Hidratos (%)',{exact:true}).fill('50');await page.getByLabel('Grasas (%)',{exact:true}).fill('30');await page.getByLabel('Fibra (g/día, opcional)',{exact:true}).fill('25');await page.getByRole('button',{name:'Confirmar mi objetivo',exact:true}).click();await page.getByLabel('Desayuno (%)',{exact:true}).fill('20');await page.getByLabel('Autorizo enviar objetivo calórico',{exact:false}).check();await page.getByRole('button',{name:'Crear plan de comidas',exact:true}).click();await expect(page.locator('.plan-recipe-title strong')).toHaveCount(7);expect(payloads).toHaveLength(7);expect(payloads[0].request.desireText).toContain('400 kcal POR PERSONA');expect(payloads[0].request.desireText).toContain('proteinG: 20.0');expect(payloads[0].request.desireText).toContain('fiberG: 5.0');expect(payloads[0].request.pantryBasics).toEqual([]);
 await page.getByRole('button',{name:'Pedir alternativa con IA',exact:true}).first().click();await expect(page.getByRole('button',{name:'Usar esta alternativa',exact:true})).toBeVisible();await expect(page.locator('.plan-recipe-title strong').first()).toHaveText('Desayuno propuesto 1');await page.getByRole('button',{name:'Usar esta alternativa',exact:true}).click();await expect(page.locator('.plan-recipe-title strong').first()).toHaveText('Desayuno propuesto 8');
});
