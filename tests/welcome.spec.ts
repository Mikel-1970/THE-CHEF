...
test('first visit shows El Chef and automatically opens login in 2–3 seconds', async ({ page }, info) => {
  ...
  await page.goto('./');
  const splash = page.getByRole('main', { name: 'Presentación de The Chef' });
...
test('animated entry and login render without clipping', async ({ page }, info) => {
  ...
  await expect(page.locator('.welcome-content p')).toHaveCSS('opacity', '1');
  ...
  await expect(page.getByRole('button', { name: 'Entrar', exact: true })).toBeVisible();
  await expect(page.locator('.access-arrival')).toHaveCSS('opacity', '1');
...
test('authenticated session goes directly to the requested route without splash or login', async ({ page }) => {
  ...
  await expect(page.locator('.welcome-splash,.access-card')).toHaveCount(0);
