...
test('retired pantry routes return to dish search without changing stored stock',async({page})=>{
 await page.addInitScript(()=>localStorage.setItem('chef:settings',JSON.stringify({pantryStock:[{name:'Arroz',quantity:1,unit:'kg',location:'pantry'}]})));
 for(const path of ['nevera','cocina-despensa']){await page.goto('./#/'+path);await expect(page).toHaveURL(/#\/antojo$/);await expect(page.getByRole('button',{name:'Buscar receta',exact:true})).toBeVisible();}
 expect(await page.evaluate(()=>JSON.parse(localStorage.getItem('chef:settings')!).pantryStock[0].name)).toBe('Arroz');
 await page.goto('./#/ajustes');await expect(page.getByLabel('Usar IA si no hay una receta adecuada para mis ingredientes')).toHaveCount(0);
});
