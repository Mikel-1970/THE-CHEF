...
test('search starts empty, uses shared filters and shows photographed results',async({page},info)=>{
 await page.goto('./#/buscar');await expect(page.locator('.library-photo-card')).toHaveCount(0);await page.getByRole('button',{name:'Filtros',exact:true}).click();await expect(page.getByLabel('Comensales',{exact:true})).toHaveCount(0);await expect(page.getByLabel('Estilo',{exact:true})).toBeVisible();await expect(page.getByLabel('Picante',{exact:true})).toBeVisible();await page.getByPlaceholder('Ej. pasta pollo, italiana, calabacín…').fill('arroz');await expect(page.locator('.library-photo-card').first()).toBeVisible();await expect(page.locator('.library-photo-card img').first()).toBeVisible();await expect(page.getByRole('button',{name:'Generar una receta con IA',exact:true})).toBeVisible();await page.screenshot({path:info.outputPath('search.png'),fullPage:true});
