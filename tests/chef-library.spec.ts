...
test('reviewed library layout and menu keep every group accessible',async({page},info)=>{
 ...
 await page.getByRole('button',{name:'Abrir menú',exact:true}).click();await expect(page.locator('.chef-menu-grid button')).toHaveText(['Inicio','Mis recetas','Favoritos','Técnicas y tips','Qué cocinar','Abre la despensa','Foto Receta','Mi plan semanalPróximamente','Lista de la compra','Despensa','Buscar','Perfil y ajustes']);
 await expect(page.getByRole('button',{name:'Mi plan semanal',exact:true})).toBeDisabled();await page.screenshot({path:info.outputPath('review-menu.png')});
});
