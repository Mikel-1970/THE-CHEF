...
test('admin enters from empty storage and keeps avatar across reload and new tab',async({page,context})=>{
 ...
 await page.goto('./');await page.getByRole('button',{name:'Login',exact:true}).click();await expect(page.locator('.reference-action-card')).toBeVisible();
 ...
 await other.getByRole('button',{name:'Login',exact:true}).click();await expect(other.locator('.reference-action-card')).toBeVisible();await expect(other.locator('.chef-draggable-avatar img')).toHaveAttribute('src',/dachshund.png$/);
...
test('expired session stops showing the application',async({page})=>{
 ...
 await page.goto('./');await page.getByRole('button',{name:'Login',exact:true}).click();await expect(page.locator('.reference-action-card')).toBeVisible();await page.clock.fastForward(61000);
